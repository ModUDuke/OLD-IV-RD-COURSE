...
# 4) As a last exercise, let's further relax the quadratic assumption of halftime margin. Instead of estimating an OLS regression using the lm function, we will now use a package called 'rdd' which allows us to examine how sensitive the regression assumptions are for our RD estimate.
...
success_msg("Good work! From the looks of it, the quadratic specification of halftime margin looks very similar to the nonparametric estimate (4.8 points versus 4.4 points, respectively). The linear specification seems to not capture the true relationship between halftime margin and final margin, as it only estimates an effect of 2.5 points.")
```
...
*** =instructions
- 1) Using the function `DCdensity` which is already on your workspace, plot the distribution of data near the cutoff
...
*** =sample_code
```{r}
# 1) Run the McCrary test with the running variable in the NBA data and the cutoff (score tied)
#---- Question 1-------------------------------------#
      Solution1 <- DCdensity(NBA$[running.var],[cutoff],plot=TRUE)
#----------------------------------------------------#
  
# 2) Now issue the command again, but print the p-value output. Put print() around the command
#---- Question 2-------------------------------------#
      Solution2 <- DCdensity(NBA$[running.var],[cutoff],plot=FALSE)
#----------------------------------------------------#

# 3) Now plot using the variable `home.team.halftime.margin.cheat` which has been manipulated
#---- Question 3-------------------------------------#
      Solution3 <- DCdensity(NBA$[new.running.var],[cutoff],plot=TRUE)
#----------------------------------------------------#

# 4) And print its p-value:
#---- Question 4-------------------------------------#
...
3. Running variable: Population. Treatment variable: Number of seats a region gets in a national government with a fixed amount of total seats (e.g., number of representatives per state out of the 345 total in the U.S. House of Representatives).
...
msg1 = "Try again"
msg2 = "Try again"
msg3 = "Correct! Often the distinction between sharp and fuzzy hinges on the definition of treatment. For (1) and (2), treatment is defined as **eligibility** rather than takeup. In contrast (3) is fuzzy because the treatment is not completely consistent for all units. Rural districts may get 1 representative for every 200,000 people, but in urban districts, 1 politician may represent 800,000 people. In a way, the way that governments create districts means they are not reacting proportionally to the changes in the population in particular regions, and it's often helpful to think of fuzzy RDDs as having a non-compliance problem."
msg4 = "Try again"
msg5 = "Try again"
msg6 = "Try again"
msg7 = "Try again"
