...
As we have mentioned, the main idea with RD is that there should not be any difference in the people on either side of the cutoff. So a big part of RD analysis is to check whether this is actually true in our data. For example, the distributions of our variables should be the same on both sides of the cutoff. If we find differences in our distributions, then we might need to worry that these differences are due to people choosing which side of the cutoff to be on.

Let's look at this by revisiting the NBA data we used before in previous exercises. In professional basketball games, manipulation might happen if teams try to be down by 1 or 2 points at halftime--perhaps because they read that research paper published in Management Science!
...
*** =sample_code
```{r}
# 1) Run the McCrary test with the running variable in the NBA data and the cutoff (score tied). Insert the correct variable names into the placeholder spots below:

      DCdensity(NBA$[running.var],[cutoff],plot=TRUE)

  
# 2) Now issue the command again (again by inserting the correct variable names in the placeholder spots), but print the p-value output. You can do this by simply wrapping the command with the print() function:

      DCdensity(NBA$[running.var],[cutoff],plot=FALSE)


# 3) Now plot using the variable `home.team.halftime.margin.cheat` which is a version that has been manipulated:

      DCdensity(NBA$[new.running.var],[cutoff],plot=TRUE)

# 4) And now print its p-value to see how it compares to our non-manipulated data:
  
      DCdensity(NBA$[new.running.var],[cutoff],plot=FALSE)
