...
- The syntax for this function is: model <- ivreg2(form = [second stage regression formula], endog = c("endogenous.regressor", iv = c("instruments"), data=name.of.data.frame)
...
#------------------------------------------
# Print statistics to compare with OHIE data
#------------------------------------------
print(paste0("Number of individuals interviewed: ",sum(OHIE$intvw)))
print(paste0("Number of individuals in lottery group: ",sum(OHIE$lottery==1,na.rm=TRUE)))
print(paste0("Number of individuals in control group: ",sum(OHIE$lottery==0,na.rm=TRUE)))
...
Solution1<-lm(pos_depression_screen~lottery+gender_inp+age_35_49_inp+age_50_64_inp+race_black_inp+race_nwother_inp+hispanic_inp+itvw_english_inp,data=OHIE)$results
